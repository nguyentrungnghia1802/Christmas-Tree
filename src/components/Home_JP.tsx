...
function Home_JP() {
  ...
  const handleOk = () => {
    // モーダルを閉じて同じソースパラメータでクリスマスツリーに移動
    setShowModal(false);
    navigate(`/jp/christmas-tree?source=${sourceIndex}`);
  };
    ...
    <div style={{
      ...
      {currentSource.name && (
        <p style={{
          ...
          🎁 <strong>{currentSource.name}</strong> への特別なプレゼント
        </p>
      )}
      <style>{`
        @keyframes twinkle {
      ...
      `}</style>
      <div style={{ cursor: 'pointer', display: 'inline-block' }} onClick={handleTreeClick} title={`3Dクリスマスツリーへ移動 - ${currentSource.name}`}> 
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ marginBottom: '15px' }}>
                ...
                <p style={{ margin: '8px 0 0 0', paddingLeft: '10px' }}>
                  読み込みが完了したらAIをオンにして、カメラへのアクセスを許可してください
                </p>
              </div>
